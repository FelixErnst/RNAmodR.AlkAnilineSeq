...
files <- list("wt" = c(treated = RNAmodR.files[["example_wt_1.bam"]],
                       treated = RNAmodR.files[["example_wt_2.bam"]],
                       treated = RNAmodR.files[["example_wt_3.bam"]]),
              "Bud23del" = c(treated = RNAmodR.files[["example_bud23_1.bam"]],
                             treated = RNAmodR.files[["example_bud23_2.bam"]]),
              "Trm8del" = c(treated = RNAmodR.files[["example_trm8_1.bam"]],
...
```{r}
coord <- mod[[1]][c(1:3,5:7,9,10)]
alias <- data.frame(tx_id = c(1,3,5,6,7,8,10,11),
...
```{r plot2, fig.cap="Heatmap showing Stop ratio scores for detected m7G1575 on the 18S rRNA.", fig.asp=0.7}
plotCompareByCoord(msaas, coord[1], score = "scoreSR", alias = alias)
```

In addition, the aggregate data along the transcript visualized as well.

```{r plot3, fig.cap="", fig.asp=1.5}
visualizeData(msaas, "1", from = 1550L, to = 1600L)
```

This includes raw data as well.

```{r plot4, fig.cap="", fig.asp=2}
visualizeData(msaas, "1", from = 1550L, to = 1600L, seqdata = TRUE)
```
